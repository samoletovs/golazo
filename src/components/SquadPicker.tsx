...
type SquadPickerMode = 'player' | 'coach'
...
export function SquadPicker({ mode, onClose }: SquadPickerProps) {
  ...
  const squads: SquadEntry[] = useMemo(() => {
    ...
    return (profile?.teams ?? []).filter((t) => t.active).map((t) => ({
      id: t.id,
      name: t.name,
      ...
      position: Array.isArray(t.position) ? t.position[0] : t.position,
    }))
  }, [profile, mode])
              ...
              <div className="card p-4">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {POSITION_OPTIONS.map(({ key, emoji }) => (
                        <button key={key} className="text-xs font-bold px-3 py-1.5 rounded-xl tap-target"
                          style={{
                            background: position === key ? 'var(--color-primary-dark)' : 'var(--color-glass-hover)',
                            color: position === key ? '#fff' : 'var(--color-text-muted)',
                          }}
                ...
                <div className="flex items-center gap-3 pt-3" style={{ borderTop: '1px solid var(--color-glass-border)' }}>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
                      {mode === 'coach' ? t(`coach.role.${coachRole}`) : position}
                    </p>
                  </div>
