...
export function AcademyShell({ page, onNavigate, children }: { page: Page; onNavigate: (page: string) => void; children: ReactNode }) {
  ...
  return (
    <div className="academy-shell" data-academy-shell={profile?.role ?? 'player'}>
      ...
      <header className="academy-masthead">
        ...
        <nav className="academy-primary-nav" aria-label={t('academy.navigation')}>
        ...
        <div className="academy-rail-identity"><strong>{profile?.role === 'player' ? profile.jerseyNumber ?? 'G' : 'G'}</strong><span>{profile?.name}</span><small>{teamName}</small></div>
      ...
      <div className="academy-workspace">
        <div className="academy-context"><span>{teamName || t('academy.yourFootball')}</span><button className="academy-player-link" onClick={() => navigate('profile')}>
          <span className="academy-shirt-number">{profile?.jerseyNumber ?? 'G'}</span><span>{profile?.name}<small>{profile?.positions?.join(' / ')}</small></span>
        </button></div>
