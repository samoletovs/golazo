...
export function TournamentImport({ onClose }: { onClose: () => void }) {
  ...
  const { profile, saveTournamentImport } = useApp()
  ...
  const [teamName, setTeamName] = useState(
    profile?.teams?.find((t) => t.isPrimary)?.name || profile?.teams?.[0]?.name || profile?.team || '',
  )
  ...
  function handleImport() {
    if (!result || !result.games.length || saving.current) return
    saving.current = true
    try {
      ...
      const receipt = saveTournamentImport(tournament, events)
      setImportedCount(receipt.addedGames)
      setError('')
      if (errorToast.current !== null) dismissToast(errorToast.current)
      setImported(true)
      void createOrJoinSharedTournament(url, result.tournament, startDate, endDate, location, result.games).catch(
        (cause) => {
          console.error('Team sharing was not confirmed after local tournament import:', cause)
          setSharingFailed(true)
          showToast(t('academy.sharingUnconfirmed'), 'error')
        },
      )
    } catch (cause) {
      console.error('Tournament fixtures could not be saved on this device:', cause)
      setError(t('academy.saveError'))
      if (errorToast.current !== null) dismissToast(errorToast.current)
  ...
  async function createOrJoinSharedTournament(
    ...
    if (res.status === 409) {
      ...
      const joined = await fetch(`/api/shared-tournaments/${id}/join`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ teamName, className: result?.className, playerId: profile?.id }),
      })
      if (!joined.ok) throw new Error(`Shared tournament join was not confirmed: ${joined.status}`)
    } else if (!res.ok) {
      throw new Error(`Shared tournament save was not confirmed: ${res.status}`)
    }
  }
        ...
        <div className="academy-dialog-flow">
          <div className="text-center py-8">
            ...
            <p className="text-sm mt-2" style={{ color: 'var(--color-text-secondary)' }}>
              {t('import.addedGames', { count: importedCount, tournament: result.tournament })}
            </p>
            <p className="academy-hint mt-4">{t('training.localHint')}</p>
            {sharingFailed && (
              <p className="academy-error mt-4" role="alert">
                {t('academy.sharingUnconfirmed')}
              </p>
            )}
          </div>
      ...
      <div className="academy-dialog-flow">
        ...
        {!result ? (
          ...
          <div className="flex flex-col gap-4">
                ...
                <div className="mt-3">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const selected = playerTeams.find((team) => team.id === e.target.value)
                      if (selected) setTeamName(selected.name)
                    }}
                    className="w-full text-sm p-2.5 rounded-lg mt-1"
                    style={{
                      background: 'var(--color-bg-field, #f8fafc)',
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {fetchedTeams.map((team) => (
                      <button
                        ...
                        style={{
                          background:
                            teamName === team ? 'rgba(var(--color-primary-rgb), 0.12)' : 'var(--color-glass-hover)',
                          color: teamName === team ? 'var(--color-primary-dark)' : 'var(--color-text-secondary)',
                          border:
                            teamName === team ? '1.5px solid var(--color-primary-dark)' : '1.5px solid transparent',
                        }}
                        onClick={() => {
                          setTeamName(team)
                          setError('')
            ...
            {error && (
              <div
                className="flex items-start gap-2 text-xs p-3 rounded-xl whitespace-pre-wrap"
            ...
            <button className="btn-primary w-full" onClick={handleFetch} disabled={loading || !url || !teamName}>
          ...
          <div className="flex flex-col gap-3">
            <div className="card p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p
                    className="text-base font-extrabold heading-display"
                ...
                return (
                  ...
                  >
                    ...
                    <span className="flex-1 font-bold truncate">
                      {homeMatch?.logoUrl && (
                        <img
                          src={homeMatch.logoUrl}
                          alt={game.home}
                          className="inline-block w-4 h-4 rounded mr-1 align-text-bottom object-contain"
                          onError={(e) => {
                            ;(e.target as HTMLImageElement).style.display = 'none'
                          }}
                        />
            ...
            <div className="flex gap-2 mt-1">
              ...
              <button className="flex-1 btn-primary" onClick={handleImport}>
